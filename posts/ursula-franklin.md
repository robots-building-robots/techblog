...
Franklin's checklist is not a prescribed formula; it is a set of questions anyone building or deploying technology should return to at every stage of the process. Who benefits from what we are building, and who is excluded? Are the people most affected by our decisions involved in making them? Are we designing for reversibility, avoiding lock-in, favouring open and interoperable systems? Are we measuring the true cost of what we ship, not only in compute and capital, but in social and ecological impact? How do we define the line between authorship and automation?

These are not questions the industry habitually asks, or asks only as superficial marketing.

These questions become only more urgent as prescriptive technologies are adopted at scale. Are we defaulting to the most convenient, most heavily marketed tool, or asking whether a lighter, more transparent, more privacy-respecting alternative exists? Are we using these tools in ways that retain human authorship and judgment, or are we just deferring to the machine's output? Are we building products that extend the user's capacity, or ones that deepen their dependency? Locally run models, open source platforms, and tools that preserve data sovereignty are not merely technical preferences. Measured against Franklin's checklist, they are the more principled choices.

None of this is straightforward in workplaces where LLM adoption is being driven from above, often without meaningful consultation. The reciprocity Franklin calls for is frequently absent, whether at the interface, workflow, or policy level. In this context, earthworming means what it has always meant: advocating within teams and organizations for more open debate, refusing uses that hollow out craft or concentrate power, supporting open alternatives where possible, and insisting that the question of how these tools are built and deployed belongs in open, ongoing discourse.
